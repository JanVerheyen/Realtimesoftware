...
int main(){
    ...
    int nDigits_height =floor(log10(abs(y_0)))+1, nDigits_velocity = floor(log10(abs(v)))+1;    //calculates how many decimals a number is
    ...
    for(i=1;i<17;i+=1){
        theta_table = i*5;
        height_table = i*(pow(10,nDigits_height-1));
        velocity_table = i*(pow(10,nDigits_velocity-1));
